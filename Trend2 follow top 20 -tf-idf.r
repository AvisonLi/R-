# TREND 2: THEMATIC ANALYSIS USING TF-IDF ON TOP 20 TFIDF IN TREND1
...
filtered_metadata <- read.csv("top20_articles.csv")
...
if ("tfidf" %in% colnames(filtered_metadata)) {
  ...
  theme_article_counts <- theme_results %>%
    group_by(year, theme, word) %>%
    summarise(
      keyword_count = sum(keyword_count), 
      .groups = "drop"
